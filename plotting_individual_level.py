...
figure_path = 'C:/Users/vjtha/OneDrive - O365 Turun yliopisto/Pro gradu/Figures/'
...
def plot_deviation_histogram(df, type):
    ...
    plt.tight_layout()
    plt.savefig(f'{figure_path}Distribution_Excess_Returns_{type}.png')
    plt.show()
...
def plot_deviation_line_chart(df, type):
    ...
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(f'{figure_path}Excess_Returns_{type}.png')
    plt.show()
...
for folder in folders:    
    ...
    for treshold in tresholds:
        ...
        for file in files:
            ...
            if folder == "nyse":
                df = df.drop(['CMRE','CMSC','CMSD','GMED','ATEN','BBU'],axis=1)
            print(df.head())
